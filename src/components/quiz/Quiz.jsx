...
function Quiz() {
  ...
  const {roll,subject,week} = useParams();
  ...
  useEffect( () => {
    ...
    const Method = async() =>{
     try{
       const res = await axios.get(`${process.env.REACT_APP_URL_QUESTION}/question/subjects/${subject}/weeks/${week}`);
...
,[])
   ...
   if(data[index].questionType.match("single")){
    ...
    const matter=  data[index].correctAnswers.map(ans=>{
                  if(ans===multiple[i]){
                    add+=value;
                  }
                  return ans;
    ...
    alert(data[index].correctAnswers)
    setScore(score+add);
    if(index === data.length-1){
      setDisplay2("block")
    }}}
   ...
   const handleCorrectAnswerChange= (e,value) =>{
    if(data[index].correctAnswers[0].match(value)){
      setScore(score+1);
    }
    else{
      setDisplay("block")
  ...
  return (
    <div className='container'>

      {
        ...
        <h1>Quiz Practice</h1>
        <hr />
        <h2>{index+1 }. <nbsp/> { data[index].question}</h2>
        
        {
              data[index].questionType === "single" && 
              (
        ...
        {
                 data[index].questionType === "multiple" && 
                 (
                   <div className='answer grid-two--cols'>
          ...
          <div style={{display:display}} className='correct'>
              ANSWER : {data[index].correctAnswers[0]}
          </div>

          <div style={{display:display2}} className='final'>
            CORRECT ANSWER  {score} OUT OF {data.length}
        ...
        <div className="index">
         <h3> {index+1} of {data.length} Questions</h3>
        </div>
        </div>
      }
